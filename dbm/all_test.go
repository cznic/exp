...
func TestFileReadAtWriteAt(t *testing.T) {
	...
	f := db.File("TestFileReadAtWriteAt")
	fsz := func() int64 {
		n, err := f.Size()
		if err != nil {
			t.Fatal(err)
...
func BenchmarkFileWrSeq(b *testing.B) {
	...
	f := db.File("BenchmarkMemFilerWrSeq")
	if err != nil {
		b.Fatal(err)
	}
