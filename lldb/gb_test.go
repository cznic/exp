...
func TestEncodeDecodeScalars(t *testing.T) {
	table := []struct{ v, exp interface{} }{
		...
		{[]byte(""), []byte{gbBytes1, 0}},
		{[]byte("f"), []byte{gbBytes1, 1, 'f'}},
		{[]byte("fo"), []byte{gbBytes1, 2, 'f', 'o'}},
		{[]byte("foo"), []byte{gbBytes1, 3, 'f', 'o', 'o'}},
		{[]byte(s256[:255]), append([]byte{gbBytes1, 0xff}, []byte(s256[:255])...)},
		{[]byte(s256), append([]byte{gbBytes2, 0x00, 0xff}, []byte(s256)...)},
		{"", []byte{gbString1, 0}},
		{"f", []byte{gbString1, 1, 'f'}},
		{"fo", []byte{gbString1, 2, 'f', 'o'}},
		{"foo", []byte{gbString1, 3, 'f', 'o', 'o'}},
